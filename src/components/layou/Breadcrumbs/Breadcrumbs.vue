<template>
  ...
  <template v-for="(item,index) in breadcrumbData" :key="item.path">
    <el-breadcrumb-item class="no-redirect" v-if="breadcrumbData.length-1 === index">{{$t(item.meta?.title)}}</el-breadcrumb-item>
    <el-breadcrumb-item v-else class="redirect" :to="item.path"> {{$t(item.meta?.title)}}</el-breadcrumb-item>
  </template>
  </el-breadcrumb>
</template>
